...
def mtl_decode(sentences, predict_dict, ner_labels_inv, stat_labels_inv, rel_labels_inv, config):
  predictions = {}
  ...
  if "stat_scores" in predict_dict:
    predictions["stat"] = [{} for i in range(num_sentences)]
  ...
  for i in range(num_sentences):
    ...
    if "stat" in predictions:
      stat_spans = _dp_decode_non_overlapping_spans(
          predict_dict["candidate_starts"][i],
          predict_dict["candidate_ends"][i],
          predict_dict["stat_scores"][i],
          len(sentences[i]), stat_labels_inv, None, False)
      predictions["stat"][i] = stat_spans
