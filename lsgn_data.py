...
# ]
_label_names = [
    "ner_starts", "ner_ends", "ner_labels", "ner_len","stat_starts", "stat_ends", "stat_labels", "stat_len",
    "rel_e1_starts", "rel_e1_ends", "rel_e2_starts", "rel_e2_ends", "rel_labels", "rel_len"
]

# Name for predicted tensors.
_predict_names = [
    "candidate_starts", "candidate_ends", "candidate_arg_scores", "candidate_pred_scores", "ner_scores", 
    "arg_scores", "pred_scores",
    "candidate_mention_starts", "candidate_mention_ends", "candidate_mention_scores", "mention_starts",
    "mention_ends",
    "srl_head_scores", "ner_head_scores", "entity_gate", 
    "candidate_stat_starts", "candidate_stat_ends", "stat_scores",
    # Relation stuff.
...
class LSGNData(object):
  def __init__(self, config):
    ...
    self.stat_labels = { l:i for i,l in enumerate([""] + config["stat_labels"]) }
    self.stat_labels_inv = [""] + config["stat_labels"]
    ...
    self.input_props = [
        ...
        (tf.int32, [None]),  # stat starts.
        (tf.int32, [None]),  # stat ends.
        (tf.int32, [None]),  # stat labels.
        (tf.int32, []),  # Number of stat spans.
  ...
  def start_enqueue_thread(self, session):
    ...
    def _enqueue_loop():
      ...
      while True:
        ...
        # print ("Load {} training documents with {} sentences, {} clusters, and {} mentions.".format(
        #     doc_id, num_sentences, cluster_id_offset, num_mentions))
        print ("Load {} training documents with {} sentences and {} mentions.".format(
            doc_id, num_sentences, num_mentions))
  ...
  def split_document_example(self, example):
    ...
    for i, sentence in enumerate(sentences):
      ...
      sent_example = {
        ...
        "stat": example["stat"][i] if "stat" in example else [],
  ...
  def tensorize_example(self, example, is_training):
    ...
    stat_starts, stat_ends, stat_labels = (
        tensorize_labeled_spans(example["stat"], self.stat_labels))
    ...
    example_tensor = {
      ...
      "stat_starts": stat_starts - word_offset,
      "stat_ends": stat_ends - word_offset,
