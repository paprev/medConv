...
class SRLModel(object):
  ...
  def get_predictions_and_loss(self, inputs, labels):
    ...
    # ##my code for stat
    # # [num_sentences, max_num_candidates], ...
    # candidate_stat_starts, candidate_stat_ends, candidate_stat_mask = get_span_candidates(
    #     text_len, max_sentence_length, self.config["max_arg_width"])
    # flat_candidate_stat_mask = tf.reshape(candidate_stat_mask, [-1])  # [num_sentences, max_num_candidates]
    # batch_word_stat_offset = tf.expand_dims(tf.cumsum(text_len, exclusive=True), 1)  # [num_sentences, 1]
    # flat_candidate_stat_starts = tf.boolean_mask(
    #     tf.reshape(candidate_stat_starts + batch_word_stat_offset, [-1]), flat_candidate_stat_mask)  # [num_candidates]
    # flat_candidate_stat_ends = tf.boolean_mask(
    #     tf.reshape(candidate_stat_ends + batch_word_stat_offset, [-1]), flat_candidate_stat_mask)  # [num_candidates]

    # # text_len_stat_mask = tf.sequence_mask(text_len, maxlen=max_sentence_length)  # [num_sentences, max_sentence_length]
    # # flat_context_outputs = flatten_emb_by_sentence(context_outputs, text_len_mask)  # [num_doc_words]
    # # flat_head_emb = flatten_emb_by_sentence(head_emb, text_len_mask)  # [num_doc_words]
    # # doc_len = util.shape(flat_context_outputs, 0)

    # candidate_stat_span_emb, head_stat_scores, span_head_stat_emb, head_indices_stat, head_indices_stat_log_mask = get_span_stat_emb(
    #     flat_head_emb, flat_context_outputs, flat_candidate_stat_starts, flat_candidate_stat_ends,
    #     self.config, self.dropout
    # )  # [num_candidates, emb], [num_candidates, max_span_width, emb], [num_candidates, max_span_width]

    # num_stat_candidates = util.shape(candidate_stat_span_emb, 0)
    # max_num_candidates_stat_per_sentence = util.shape(candidate_stat_mask, 1)
    # candidate_span_stat_ids = tf.sparse_to_dense(
    #     sparse_indices=tf.where(tf.equal(candidate_stat_mask, True)),
    #     output_shape=tf.cast(tf.stack([num_sentences, max_num_candidates_stat_per_sentence]), tf.int64),
    #     sparse_values=tf.range(num_stat_candidates, dtype=tf.int32),
    #     default_value=0,
    #     validate_indices=True)  # [num_sentences, max_num_candidates]
    ...
    ##my code for stat
    # spans_log_stat_mask = tf.log(tf.to_float(candidate_stat_mask))  # [num_sentences, max_num_candidates]
    # predict_dict = {"candidate_stat_starts": candidate_stat_starts, "candidate_stat_ends": candidate_stat_ends}
    ...
    # if head_stat_scores is not None:
    #   predict_dict["head_stat_scores"] = head_stat_scores
    ...
    if self.config["ner_weight"]:
      ...
      gold_ner_labels = get_span_task_labels(
          candidate_starts, candidate_ends, labels, max_sentence_length)  # [num_sentences, max_num_candidates]


    if self.config["stat_weight"]:
      # gold_ner_labels, gold_coref_cluster_ids = get_span_task_labels(
      #     candidate_starts, candidate_ends, labels, max_sentence_length)  # [num_sentences, max_num_candidates]
      print('labels',labels)
      gold_stat_labels = get_span_stat_task_labels(
          candidate_starts, candidate_ends, labels, max_sentence_length)  # [num_sentences, max_num_candidates]


    if self.config["relation_weight"] > 0:
      print('self.data.rel_labels',self.data.rel_labels)
    ...
    print('self.config["ner_weight"]',self.config["ner_weight"])
    print('self.config["stat_weight"]',self.config["stat_weight"])
    print('self.config["relation_weight"]',self.config["relation_weight"])
    ...
    if self.config["ner_weight"] > 0:
      print('self.data.ner_labels',self.data.ner_labels)
    ...
    ###my code for stat
    dummy_stat_scores = tf.expand_dims(tf.zeros_like(candidate_span_ids, dtype=tf.float32), 2)
    if self.config["stat_weight"] > 0:
      print('self.data.stat_labels',self.data.stat_labels)
      flat_stat_scores = get_unary_scores(
          candidate_span_emb, self.config, self.dropout, len(self.data.stat_labels) - 1,
          "stat_scores")  # [num_candidates, num_labels-1]
      if self.config["span_score_weight"] > 0:
        #flat_ner_scores += self.config["span_score_weight"] * tf.expand_dims(flat_span_scores, 1)
        flat_stat_scores += self.config["span_score_weight"] * tf.expand_dims(flat_candidate_entity_scores, 1)
      stat_scores = tf.gather(
          flat_stat_scores, candidate_span_ids
      ) + tf.expand_dims(spans_log_mask, 2)  # [num_sentences, max_num_candidates, num_labels-1]
      stat_scores = tf.concat([dummy_stat_scores, stat_scores], 2)  # [num_sentences, max_num_candidates, num_labels]
      print('gold_stat_labels',gold_stat_labels)
      stat_loss = get_softmax_loss(stat_scores, gold_stat_labels, candidate_mask)  # [num_sentences]
      stat_loss = tf.reduce_sum(stat_loss) # / tf.to_float(num_sentences)  # []
      predict_dict["stat_scores"] = stat_scores
    else:
      stat_loss = 0
    
    print('test_5')
    tf.summary.scalar("NER_loss", ner_loss)
    tf.summary.scalar("STAT_loss", stat_loss)
    ...
    loss = self.config["ner_weight"] * ner_loss + (self.config["stat_weight"] * stat_loss +
        self.config["relation_weight"] * rel_loss)
