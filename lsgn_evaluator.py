...
class LSGNEvaluator(object):
  ...
  def evaluate(self, session, data, predictions, loss, official_stdout=False):
    if self.eval_data is None:
      ...
      # self.eval_data, self.eval_tensors = data.load_eval_data()
    # print('self.eval_data',self.eval_data)
    def _k_to_tag(k):
      if k == -3:
        return "oracle"
      elif k == -2:
    ...
    for i, doc_tensors in enumerate(self.eval_tensors):
      ...
      predict_names = []
      # print('data.predict_names',data.predict_names)
      # print('len(data.predict_names)',len(data.predict_names))
      for tn in data.predict_names:
        if tn in predictions:
          predict_names.append(tn)
      # print('predict_names',predict_names)
      # print('len(predict_names)',len(predict_names))
      ...
      # doc_example = self.eval_data[i]
      # print('doc_example',doc_example)
      sentences = doc_example["sentences"]
      decoded_predictions = inference_utils.mtl_decode(
          sentences, predict_dict, data.ner_labels_inv, data.stat_labels_inv, data.rel_labels_inv,
          self.config)

      # Relation extraction.
      if "rel" in decoded_predictions:
        rel_predictions.extend(decoded_predictions["rel"])
        # print('predict_dict.keys()',predict_dict.keys())
      ...
      if "stat" in decoded_predictions:
        stat_predictions.extend(decoded_predictions["stat"])
    ...
    sentences, gold_srl, gold_ner, gold_relations, gold_stat = zip(*self.eval_data)
    ...
    if self.config["stat_weight"] > 0:
      print('FOR STAT')
      stat_precision,stat_recall,stat_f1,ul_stat_prec,ul_stat_recall,ul_stat_f1,stat_label_mat = (
          srl_eval_utils.compute_span_f1_stat(gold_stat, stat_predictions, "STAT"))
      summary_dict["STAT F1"] = stat_f1
      summary_dict["STAT precision"] = stat_precision
      summary_dict["STAT recall"] = stat_recall
      summary_dict["Unlabeled STAT F1"] = ul_stat_f1
      summary_dict["Unlabeled STAT precision"] = ul_stat_prec
      summary_dict["Unlabeled STAT recall"] = ul_stat_recall

      # Write NER prediction to IOB format and run official eval script.
      srl_eval_utils.print_to_iob2(sentences, gold_stat, stat_predictions, self.config["ner_conll_eval_path"])
      task_to_f1["stat"] = stat_f1
